...
cityForm.addEventListener("submit", function (e) {
  ...
  $.ajax({
    ...
    //Day 1 forecast
    $("#dayOneDate").text(response.list[0].dt_txt.slice(0, 10));
    var tempF1 = (response.list[0].main.temp_max - 273.15) * 1.8 + 32;
    $("#dayOneTemp").text(tempF1.toFixed(2) + " F");
    $("#dayOneHumidity").text(response.list[0].main.humidity + "%");
    //Day 2 forecast
    $("#dayTwoDate").text(response.list[7].dt_txt.slice(0, 10));
    var tempF2 = (response.list[7].main.temp_max - 273.15) * 1.8 + 32;
    $("#dayTwoTemp").text(tempF2.toFixed(2) + " F");
    $("#dayTwoHumidity").text(response.list[7].main.humidity + "%");
    //Day 3 forecast
    $("#dayThreeDate").text(response.list[15].dt_txt.slice(0, 10));
    var tempF3 = (response.list[15].main.temp_max - 273.15) * 1.8 + 32;
    $("#dayThreeTemp").text(tempF3.toFixed(2) + " F");
    $("#dayThreeHumidity").text(response.list[15].main.humidity + "%");
    //Day 4 forecast
    $("#dayFourDate").text(response.list[23].dt_txt.slice(0, 10));
    var tempF4 = (response.list[23].main.temp_max - 273.15) * 1.8 + 32;
    $("#dayFourTemp").text(tempF4.toFixed(2) + " F");
    $("#dayFourHumidity").text(response.list[23].main.humidity + "%");
    //Day 5 forecast
    $("#dayFiveDate").text(response.list[31].dt_txt.slice(0, 10));
    var tempF5 = (response.list[31].main.temp_max - 273.15) * 1.8 + 32;
    $("#dayFiveTemp").text(tempF5.toFixed(2) + " F");
    $("#dayFiveHumidity").text(response.list[31].main.humidity + "%");
  });
});
